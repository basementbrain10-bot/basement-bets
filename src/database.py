...
# -------------------------------------------------------------------
# Async SQLAlchemy Support (Added for NCAAM Model Refactor)
# -------------------------------------------------------------------
try:
    ...
    def get_async_db_url():
        """Ensure URL uses asyncpg driver and strips incompatible params."""
        ...
        # Asyncpg doesn't support sslmode in URL, remove it.
        if "?" in url:
            base, qs = url.split("?", 1)
    ...
    def init_async_db():
        ...
        if not async_engine:
            ...
            if url:
                # Pass ssl context explicitly if needed, or rely on defaults.
                # For Vercel/Neon/Supabase, usually explicit ssl=require is needed in connect_args
                # if we stripped it from URL.
                async_engine = create_async_engine(
                    url, 
                    echo=False, 
                    future=True,
                    # pool_pre_ping=True
                    # Verified fix for asyncpg sslmode error:
                    # We pass nothing here and let asyncpg defaults work or if explicit SSL needed:
    ...
    async def get_async_db():
        """
        Async dependency for FastAPI/Model calls.
        Usage: async for db in get_async_db(): ...
        """
        ...
        if AsyncSessionLocal:
            ...
            raise RuntimeError("Could not initialize Async DB (Missing URL?)")

except ImportError:
    print("SQLAlchemy or asyncpg not installed. Async DB support disabled.")
    async def get_async_db():
        raise NotImplementedError("Async DB support requires sqlalchemy and asyncpg")
